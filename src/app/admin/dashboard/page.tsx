          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_auto] gap-3 items-end">
            ...
            {(dateFrom || dateTo) && (
              <button
                ...
                className="text-xs text-slate-400 hover:text-white px-3 py-2.5 min-h-11 rounded-xl border border-slate-800 sm:border-0"
              >
                Clear
              </button>
            )}
...
function Kpi({
  ...
  return (
    <div className={`glass-panel p-4 sm:p-5 rounded-2xl border ${accent} space-y-2 min-w-0`}>
      <div className="flex items-center justify-between gap-2">
        <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">{label}</p>
        {icon}
      </div>
      <p className={`text-xl sm:text-3xl font-black font-mono money-break leading-tight ${valueClass}`}>
        {value}
      </p>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2 max-h-56 overflow-y-auto">
          {items.map((item) => (
            <div
              key={item.id}
              className="flex justify-between gap-2 p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs min-w-0"
            >
              <div className="min-w-0">
                <p className="font-semibold text-white truncate">{item.title}</p>
                <p className="text-slate-500 truncate">{item.meta}</p>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <span className={`font-mono font-bold whitespace-nowrap ${item.cls}`}>
