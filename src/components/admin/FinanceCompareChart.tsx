...
export function FinanceCompareChart({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-end gap-3 sm:gap-4 h-52">
        {data.map((m) => (
          <div key={m.key} className="flex-1 flex flex-col items-center gap-2 h-full justify-end min-w-0">
            <div className="w-full flex items-end justify-center gap-0.5 sm:gap-1 h-full">
              <Bar h={Math.round((m.revenue / max) * 100)} color="bg-white" title={`${revenueLabel} ${formatINR(m.revenue)}`} />
              <Bar h={Math.round((m.expenses / max) * 100)} color="bg-rose-500" title={`Expense ${formatINR(m.expenses)}`} />
              <Bar
            ...
            <p className="text-[10px] font-bold text-slate-300 text-center truncate w-full">{m.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-[11px] text-left">
          ...
          <tbody className="divide-y divide-slate-800/80">
            {data.map((m) => (
              <tr key={m.key}>
                <td className="py-1.5 pr-3 text-slate-300 font-semibold">{m.label}</td>
                <td className="py-1.5 pr-3 text-white font-mono">{formatINR(m.revenue)}</td>
                <td className="py-1.5 pr-3 text-rose-400 font-mono">{formatINR(m.expenses)}</td>
                <td className={`py-1.5 font-mono font-bold ${profitLossColor(m.profit)}`}>
